...
@Slf4j
@NoArgsConstructor
public abstract class FileCacheIndexRepositoryBase<T extends InputStreamIdentifier<?>> {
    ...
    protected FileCacheIndexRepositoryBase(final CacheConfiguration cacheConfiguration) {
        this.cacheConfiguration = cacheConfiguration;
    }

    protected void ensureCacheDirectoryResourcesExists() {

        final var cacheRoot = new File(cacheConfiguration.getDirectoryRootUri(DIRECTORY_ROOT_URI_SCHEME));
        final var cacheResources = new File(URI.create(String.format(
            "%s/%s",
            cacheConfiguration.getDirectoryRootUri(DIRECTORY_ROOT_URI_SCHEME),
    ...
    protected void cleanCacheResourceDirectory() throws IOException {
        ...
        for (final var file : Optional.ofNullable(new File(cacheConfiguration.getDirectoryRootUri(DIRECTORY_ROOT_URI_SCHEME)).listFiles()).orElse(new File[0])) {
            if (file.isDirectory()) {
